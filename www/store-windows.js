...
(function() {
    ...
    InAppBilling.prototype.init = function(success, fail, options, skus) {
        ...
        try {
            //Setup the Windows Store stuff here
            //store.win_currentApp = Windows.ApplicationModel.Store.CurrentApp;
            
            //Use this one for testing
            store.win_currentApp = Windows.ApplicationModel.Store.CurrentAppSimulator;
            
            store.win_licenseInformation = store.win_currentApp.licenseInformation;
        }
        catch(ex){
            fail();
            return;
    ...
    InAppBilling.prototype.buy = function(success, fail, productId) {
        ...
        return store.win_currentApp.requestProductPurchaseAsync(productId).then(success, fail);
    };
    ...
    try {
        store.windows = window.inappbilling;
    } catch (e) {}
})();
...
(function() {
    ...
    function init() {
        ...
        store.windows.init(iabReady, function(err) {
            initialized = false;
            store.error({
    ...
    function iabGetPurchases() {
        store.windows.getPurchases(function(purchases) {
    ...
    store.when("requested", function(product) {
        store.ready(function() {
            ...
            store.windows[method](function(data) {
                setProductData(product, data);
            }, function(err, code) {
                store.log.info("windows -> " + method + " error " + code);
    ...
    store.when("product", "finished", function(product) {
        ...
        if (product.type === store.CONSUMABLE) {
            product.transaction = null;
            store.windows.consumePurchase(function() {
